...
<div class="container">
    ...
    <div class="d-flex justify-content-end">
        <a href="${root }/main" class="mx-3">홈으로</a> | <a
            href="${root }/auth?action=member-list&currentPage=1" class="mx-3">멤버목록</a>
        |

        <c:if test="${empty loginUser }">
            <a href="${root }/member?action=login-form" class="mx-3">로그인</a> |
            <a href="${root }/member?action=regist-member-form"
               class="mx-3">회원가입</a>
        </c:if>
        <c:if test="${!empty loginUser }">
            <a href="${root }/auth?action=member-detail&email=${loginUser.email}"
