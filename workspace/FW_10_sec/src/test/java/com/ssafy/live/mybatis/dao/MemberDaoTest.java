...
public class MemberDaoTest {
    ...
    public void insertTest() {
        Assertions.assertTrue(Proxy.isProxyClass(mDao.getClass())); // 역시 SpringAOP를 구현하지는 않
        Member member = Member.builder().email("someone@email.com").name("hong").password("1234").build();
        int result = mDao.insert(member);
        Assertions.assertEquals(1, result);
        Assertions.assertNotEquals(0, member.getMno());
    }
    ...
    public void getTotalCountTest() {
        ...
        cnt = mDao.getTotalCount(new SearchCondition("name", "01", 0));
        Assertions.assertEquals(11, cnt);
    }
